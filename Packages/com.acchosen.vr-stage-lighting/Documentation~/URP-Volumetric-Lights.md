...
> **Status:** Active implementation on the `urp-volumetric-lights` branch. The DMX and AudioLink raymarched volumetric passes have landed (clean uniform density, multi-light, screen-space depth occlusion, half-res with bilateral upsample). The volumetric pass runs whenever its shader is assigned on the manager — there is no separate enable toggle, since the GPU prefabs intentionally drop the legacy mesh-cone shader and removing the volumetric pass would leave no cone visualisation at all. Realtime light components own their full configuration directly (no `*_Static` sibling inheritance) and can drive the fixture body's emissive via the `fixtureShellRenderers` list. Modulated noise density, wash mover tuning, and new GPU prefab variants are still pending — see *Phasing*. The companion document `URP-Realtime-Lights.md` describes the surface-illumination pipeline this work builds on.
